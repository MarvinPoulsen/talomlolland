...
const MovingPatternsPage: FC = () => {
  ...
  const [movingPatternsData, setMovingPatternsData] = useState([]);
  const onMapReady = (mm) => {
    minimap.current = mm;
...
// SUGGESTED MAX
  ...
  years.forEach((element) => {
    ...
    const fraflyttereTotal: number = filteredData.reduce((total, currentItem) => (total = total + parseInt(currentItem.fraflyttere || 0)), 0);
    const tilflyttereTotal: number = filteredData.reduce((total, currentItem) => (total = total + parseInt(currentItem.tilflyttere || 0)), 0);
