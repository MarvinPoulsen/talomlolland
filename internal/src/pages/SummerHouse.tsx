...
const SummerHousePage: FC = () => {
  ...
  const onChartHover = (omraade?: SummerhouseRow) => {
    if (omraade) {
      minimap.current
        .getMapControl()
        .setMarkingGeometry(omraade.shape_wkt, false, null, 3000);
    } else {
  ...
  const onChartClick = (omraade?: SummerhouseRow) => {
    if (omraade) {
      minimap.current
        .getMapControl()
        .setMarkingGeometry(omraade.shape_wkt, true, null, 2500);
    } else {
      minimap.current.getMapControl().setMarkingGeometry();
    }
  };
  const tommeSommerhusgrundeSum: number = summerhouseData.reduce(
    (total, currentItem) =>
      (total = total + parseInt(currentItem.tomme_sommerhusgrunde_count || 0)),
    0
  );
  ...
  const ubeboedeSommerhusSum: number = summerhouseData.reduce(
    (total, currentItem) =>
      (total = total + parseInt(currentItem.ubeboede_count || 0)),
    0
  );
  const beboedeSommerhusSum: number = summerhouseData.reduce(
    (total, currentItem) =>
      (total = total + parseInt(currentItem.beboede_count || 0)),
    0
  );
              ...
              <div className='block'>
                <div className='content'>
                  ...
                  <p id='total-summerhouse'>
                    I Lolland Kommune findes {sommerhusSum} sommerhuse.
                  </p>
                  <p id='empty-summerhouse'>
                    Desuden findes {tommeSommerhusgrundeSum} tomme
                    sommerhusgrunde i allerede lokalplanlagte sommerhusområder.
                  </p>
                  <p>
                    Bebyggelsen i sommerhusområderne reguleres dels af
                    lokalplaner og dels af deklarationer.
                  </p>
                </div>
              </div>
            </div>
