...
import StackedbarNoLegend, {
  StackedDataSeries,
} from '../components/chartjs/StackedbarNoLegend';
import LegendTableMulti, {
  LegendTableData,
} from '../components/chartjs/LegendTableMulti';
import PiechartNoLegend, {
  PiechartData,
} from '../components/chartjs/PiechartNoLegend';
...
const createTablesData = (data, areaCodes, analysisParams, date) => {
  ...
  for (let i = 0; i < analysisParams.length; i++) {
    ...
    for (let i = 0; i < areaCodes.length; i++) {
      ...
      const filteredData =
        data.length > 0 &&
        data.filter((row) => row.omraade === areaCode && row.dato === date);
      const value: number =
        filteredData && parseInt(filteredData[0][analysisParam.code]);
      tableSum += value;
      values.push(value);
    }
...
const HousingMarketPage: FC = () => {
  ...
  // DATA
  // Boligtype
  const housingTypesTableData = createLegendTableData(
    estateData,
    areaCodes,
    housingTypes,
    date
  ...
  const housingTypesStackedData = createStackedbarData(
    estateData,
    areaCodes,
    housingTypes
  );
  ...
  const housingTypesPiechartData = createPiechartDataSeries(
    estateData,
    areaCodes,
    housingTypes
  );
  ...
  // Ejerforhold
  const ownershipTableData = createLegendTableData(
    estateData,
    areaCodes,
    ownerships,
    date
  ...
  const ownershipsStackedData = createStackedbarData(
    estateData,
    areaCodes,
    ownerships
  );
  ...
  const ownershipsPiechartData = createPiechartDataSeries(
    estateData,
    areaCodes,
    ownerships
  );
  ...
  // Boligstørrelse
  const housingSizeTableData = createLegendTableData(
    estateData,
    areaCodes,
    housingSizes,
    date
  ...
  const housingSizeStackedData = createStackedbarData(
    estateData,
    areaCodes,
    housingSizes
  );
  ...
  const housingSizePiechartData = createPiechartDataSeries(
    estateData,
    areaCodes,
    housingSizes
  );
  ...
  for (let i = 0; i < areaCodes.length; i++) {
    ...
    housingSizePiechart.push(
      <div className="column is-3 housingSize-piechart" key={areaCode}>
        <PiechartNoLegend
          data={housingSizePiechartData[areaCode]}
          visibility={housingSizes.map((item) => item.on)}
        />
      </div>
    );
  ...
  // Boligalder
  const housingAgeTableData = createLegendTableData(
    estateData,
    areaCodes,
    housingAges,
    date
  ...
  const housingAgeStackedData = createStackedbarData(
    estateData,
    areaCodes,
    housingAges
  );
  ...
  const housingAgePiechartData = createPiechartDataSeries(
    estateData,
    areaCodes,
    housingAges
  );
  ...
  for (let i = 0; i < areaCodes.length; i++) {
    ...
    housingAgePiechart.push(
      <div className="column is-3 housing-age-piechart" key={areaCode}>
        <PiechartNoLegend
          data={housingAgePiechartData[areaCode]}
          visibility={housingAges.map((item) => item.on)}
        />
      </div>
    );
  ...
  // Husstand størrelse
  const householdSizeTableData = createLegendTableData(
    estateData,
    areaCodes,
    householdSizes,
    date
  ...
  const householdSizeStackedData = createStackedbarData(
    estateData,
    areaCodes,
    householdSizes
  );
  ...
  const householdSizePiechartData = createPiechartDataSeries(
    estateData,
    areaCodes,
    householdSizes
  );
  ...
  // Udlejningsforhold
  const rentalConditionTableData = createLegendTableData(
    estateData,
    areaCodes,
    rentalConditions,
    date
  ...
  const rentalConditionStackedData = createStackedbarData(
    estateData,
    areaCodes,
    rentalConditions
  );
  ...
  const rentalConditionPiechartData = createPiechartDataSeries(
    estateData,
    areaCodes,
    rentalConditions
  );
  ...
  const suitableForHabitation = createTablesData(
    estateData,
    areaCodes,
    suitableForHabitationParams,
    date);
...
const housingStatus = createTablesData(
  estateData,
  areaCodes,
  housingStatusParams,
  date);
