...
export interface AccidentsRow {
    aar: string;
    alv_tilskadekomne: string;
    areal_ha: string;
    bloede_trafikanter: string;
    draebte: string;
    id: string;
    let_tilskadekomne: string;
    navn: string;
    sognekode: string;
    uhelds_count: string;
    uskadte: string;
...
const getYears = (data: AccidentsRow[]) => {
    const uniqueYears = [...new Set(data.map((item) => parseInt(item.aar)))];
...
const AccidentsPage: FC = () => {
    ...
    const [accidentsData, setAccidentsData] = useState<AccidentsRow[]>([]);
    const [parishCode, setParishCode] = useState('');
    const [years, setYears] = useState<number[]>([]);
    const [involvedData, setInvolvedData] = useState<InvolvedRow[]>([]);
    const onMapReady = (mm) => {
        minimap.current = mm;
        ...
        dsInvolved.execute({ command: 'read' }, function (involvedRows: InvolvedRow[]) {
    ...
    for (let i = 0; i < years.length; i++) {
        ...
        sumParishData.push({
            ...
            alv_tilskadekomne: accidentsData
                .filter((row) => parseInt(row.aar) === year)
    ...
    const parishData = accidentsData.filter((row) => row.sognekode === parishCode);
    const parishInvolvedData =
        parishCode && parishCode !== '' ? involvedData.filter((row) => row.sognekode === parishCode) : involvedData;
    ...
    const title: string = parishCode && parishCode !== '' ? parishData[0].navn + ' Sogn' : 'Hele Lolland Kommune';
    const categories: string[] = years.map(String);
    const dataSeries: StackedDataSeries[] = [
        {
            name: 'Uskadte',
            values:
                parishCode && parishCode !== ''
                    ? parishData.map((row) => parseInt(row.uskadte))
        ...
        },
        {
            ...
            values:
                parishCode && parishCode !== ''
                    ? parishData.map((row) => parseInt(row.let_tilskadekomne))
        ...
        },
        {
            ...
            values:
                parishCode && parishCode !== ''
                    ? parishData.map((row) => parseInt(row.alv_tilskadekomne))
        ...
        },
        {
            ...
            values:
                parishCode && parishCode !== ''
                    ? parishData.map((row) => parseInt(row.draebte))
        ...
        },
        {
            ...
            values:
                parishCode && parishCode !== ''
                    ? parishData.map((row) => parseInt(row.bloede_trafikanter))
                    : sumParishData.map((row) => row.bloede_trafikanter),
            stack: '1',
        },
    ];
    const selectedParish = accidentsData.find((item) => item.sognekode === parishCode);
    const description1 =
            ...
            <div id="accidents-tab-content" className="container hidden">
                <div className="block">
                    <div className="columns">
                        <Map id={accidentsMinimapId} name="accidents" size="is-6" infoDiv="infoview" onReady={onMapReady} />
                        <div id="infoview"></div>
                                    ...
                                    <div className="block">
                                        ...
                                        <div className="columns">
                                            <div className="column is-half">
                                                <Table headers={['Tilstand', 'Antal']} data={alcoholData} />
                                            </div>
                                            <div className="column content">
                                                <p>
                                                    Gruppen af "Ædru" indeholder både personer der er testet for alkohol
                                                    påvirkning, hvor resultatet var nul og personer der ikke er blevet testet.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
