...
const DemographicsPage: FC = () => {
  ...
  const parishData = demographicsData.filter(
  ...
  for (let i = 0; i < ageGroups.length; i++) {
    ...
    lollandData.push({
      ...
      value: demographicsData
        .filter(
          (row) => row.aldersgruppe === ageGroup.title && row.dato === selectedDate
        )
        .reduce((sum, cur) => sum + parseInt(cur.antal), 0),
      on: ageGroup.on,
    });
  ...
  for (let i = 0; i < ageGroups.length; i++) {
    ...
    pyradmidData.push({
      ...
      male: parishData
        .filter(
          (row) => row.aldersgruppe === ageGroup.title && 
            row.koen === 'M' && 
            row.dato === selectedDate
        )
    ...
    parishMaleAge.push(
      parishData
        .filter(
          (row) =>
            row.aldersgruppe === ageGroup.title &&
            row.koen === 'M' && 
            row.dato === selectedDate
        )
        .reduce((sum, cur) => sum + parseInt(cur.antal) * -1, 0)
  ...
  for (let i = 0; i < dates.length; i++) {
    ...
    citizens.push(
      demographicsData
        .filter((row) => row.dato === datefilter)
        .reduce((sum, cur) => sum + parseInt(cur.antal), 0)
    );
    parishMale.push(
      demographicsData
...
const dataSeriesParishSum: LinechartDataSeries[] =[
  ...
  for (let i=0;i<parishList.length;i++){
    ...
    femalePreschool: females.filter((row) =>  row.aldersgruppe === '0-5').reduce((sum, cur) => sum + parseInt(cur.antal), 0),
    femaleSchooler: females.filter((row) =>  row.aldersgruppe === '6-17').reduce((sum, cur) => sum + parseInt(cur.antal), 0),
    femaleAdult: females.filter((row) =>  row.aldersgruppe === '18-64').reduce((sum, cur) => sum + parseInt(cur.antal), 0),
    femaleSenior: females.filter((row) =>  row.aldersgruppe === '65-79').reduce((sum, cur) => sum + parseInt(cur.antal), 0),
  ...
  const selectedParish = demographicsData.find(
    ...
    <>
      <div id="demography-tab-content" className="container hidden">
        ...
        <article className="message is-dark">
          <div className="message-header">
            <p>
              Aktuelt antal borgere i Lolland Kommune fordelt på aldersgrupper
            </p>
          </div>
          <div className="message-body">
            <div className="columns">
              <div className="block column is-half">
                <div className="columns">
                  <div className="column is-4">
                    <PiechartNoLegend
                      data={lollandData}
                      visibility={lollandData.map((item) => item.on)}
                    />
                  </div>
        ...
        <div className="block">
          <div id="demography-table" className="block">
            <TableHighligtRow
              ...
              headers={['Sogn', '0-5 år', '6-17 år', '18-64 år', '65-80 år', '+80 år', 'I alt', '0-5 år', '6-17 år', '18-64 år', '65-80 år', '+80 år', 'I alt', 'SUM']}
